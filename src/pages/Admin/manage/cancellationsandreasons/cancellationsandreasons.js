...
import { cancellationsAndReasons, vendors } from '../../../../shared/hooks/api/testService.js';
...
function CancellationsAndReasons() {
  ...
  useEffect(
    async () => {
      const data = await cancellationsAndReasons()
      ...
      <section>
        <Card className="paddingNone" style={customCardStyle}>
          <div className="card-padded-title multiple">
            <h3 className="hoverPointer">All cancellations & reasons</h3>
          </div>
          <hr
            style={{
...
export default CancellationsAndReasons;
